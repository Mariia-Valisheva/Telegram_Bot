...
def start_message_handler(message: types.Message):
    ...
    bot.send_message(
        ...
        "Кликни на нужное число пользователей👇🏻",
...
def message_handler(message: types.Message):
    ...
    if message.text == "1️⃣":
        ...
        bot.send_message(chat_id=message.chat.id, text="Не понимаю, о чем ты :(")
        return
    ...
    bot.send_message(
        chat_id=message.chat.id,
        text=f"Данные тестовых пользователей:\n<code>"\
        f"{payload_str}</code>"
    )
    bot.send_message(
        chat_id=message.chat.id,
        text="Если нужны еще пользователи, кликай снова 👇🏻",
        reply_markup=main_menu_reply_markup
    )
